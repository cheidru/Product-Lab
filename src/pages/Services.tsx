    ...
    <div>
      ...
      <section style={{ background: '#271826', color: '#fff' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '72px 48px 80px' }}>
          ...
          <h1 style={{ fontSize: 72, lineHeight: 1.0, fontWeight: 800, letterSpacing: '-0.03em', margin: '0 0 24px', maxWidth: 920 }}>
            От идеи до готового<br />к производству продукта
          </h1>
          <p style={{ fontSize: 21, lineHeight: 1.5, color: '#cbbcc4', maxWidth: 620, margin: 0 }}>
      ...
      {/* Services list */}
      <section style={{ maxWidth: 1280, margin: '0 auto', padding: '24px 48px 80px' }}>
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {services.map((s, i) => (
            <div
              ...
              style={{
                display: 'grid',
                gridTemplateColumns: '90px 1fr 1.3fr',
                gap: 36,
                padding: '40px 0',
            ...
            >
              <div style={{ fontSize: 18, fontWeight: 700, color: '#FF6A3D' }}>{s.n}</div>
              <h3 style={{ fontSize: 32, fontWeight: 700, margin: 0, letterSpacing: '-0.02em' }}>{s.title}</h3>
              <p style={{ fontSize: 17, lineHeight: 1.6, color: '#6B5E63', margin: 0 }}>{s.desc}</p>
            </div>
          ))}
        </div>
      </section>
